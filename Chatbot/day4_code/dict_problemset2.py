...
#     if "lecturer" ==  i or "manager" == i:
#         print(ssafy["classes"]["seoul"].values())

# # 선생님답

for i in range(1,3):
    print(list(ssafy.get('classes').get('seoul').values())[i])
...
for k in range(len(list(ssafy["language"]["python"]["frameworks"].keys()))):
    i = list(ssafy["language"]["python"]["frameworks"].keys())[k]
    j = list(ssafy["language"]["python"]["frameworks"].values())[k]
    something="{0}는 {1}이다.".format(i,j)
    print(something)
...
print(random.choice(list(ssafy["classes"]["seoul"]["groups"]["A"])))